 ...
 *
 *   RM → [ ANALYSIS box: Genie · Lakebase ]
 *      → Propose action → fork{ Send offer, Update the Radar }
  ...
  return (
    <section className="rounded-xl border border-border bg-card p-5 sm:p-6 overflow-hidden">
      ...
      <p className="text-sm text-muted-foreground leading-relaxed mb-5">
        ...
        Genie investigates the book, you approve the drafts, then the agent
        commits the retention + cross-sell offers, updating Lakebase live. One
        conversation, end to end.
      </p>

      {/* the loop — desktop */}
      <div className="hidden md:flex items-center justify-center" style={{ gap: 0, paddingBottom: 12 }}>
        <Stage tileSize={50} iconSize={32} icon={<OperatorIcon />} name="RM" sub="asks the question" />
        ...
        >
          ...
          <MiniTile icon={<LakebaseIcon />} label="Lakebase · the RM Radar" />
        </div>

        <Connector width={56} centered />
        <Stage tileSize={50} iconSize={32} icon={<ProposeIcon />} name="Propose action" sub="RM approves" />
        <Fork />
        ...
        >
          <Stage tileSize={50} iconSize={32} icon={<SendIcon />} name="Send offer" />
          <Stage tileSize={50} iconSize={32} icon={<LakebaseIcon />} name="Update the Radar" sub="write Lakebase" />
        </div>
      </div>

      {/* the loop — mobile: stack vertically without animated connectors */}
      <ol className="md:hidden flex flex-col gap-3 mt-2 mb-2">
        <MobileStep icon={<OperatorIcon />} label="RM" sub="asks the question" />
        ...
        <MobileStep icon={<ProposeIcon />} label="Propose action" sub="RM approves" />
        <MobileStep icon={<SendIcon />} label="Send offer" />
        <MobileStep icon={<LakebaseIcon />} label="Update the Radar" sub="write Lakebase" />
      </ol>
