...
import {
  buildAgent as buildRetentionAgent,
  configureAgentsSdk,
  run as runAgent,
  type AgentContext,
} from '../agent/retentionops.js';
...
export async function streamAgentTurn(args: {
  ...
  res: Response;
  userEmail: string;
  /** Genie space id the `ask_genie` tool queries (see retentionops.ts). */
  genieSpaceId: string;
  ...
  modelError: import('../agent/retentionops.js').ModelErrorDetail | null;
}> {
  ...
  const modelError: { current: import('../agent/retentionops.js').ModelErrorDetail | null } = {
    current: null,
  };
  ...
  try {
    ...
    const agent = buildRetentionAgent(ctx);
